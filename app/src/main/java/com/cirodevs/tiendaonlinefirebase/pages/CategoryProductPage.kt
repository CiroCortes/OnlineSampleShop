...
import com.cirodevs.tiendaonlinefirebase.components.ProductItemView
...
fun CategoryProductPage(modifier: Modifier = Modifier, categoryId: String) {
    ...
    LaunchedEffect(key1 = Unit) {
        Firebase.firestore.collection("data").document("stock")
            .collection("products")
            .whereEqualTo("category", categoryId) // aqui hacemos el filtro de la categoria
            .get().addOnCompleteListener() {
                if (it.isSuccessful) {
                    val resultList = it.result.documents.mapNotNull { doc ->
                        doc.toObject(ProductModel::class.java)
                    }
                    productList.value = resultList //.plus(resultList).plus(resultList) // solo para renderisar mas item
                }
            }
    }
